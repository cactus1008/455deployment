...
import {
  buildFraudMlPayload,
  fraudScoresFromMlResult,
  runMlPredictAsApi,
} from "./pipelinePredict";
...
export async function scoreOrderFraud(orderId) {
  ...
  const [{ data: customer }, { data: shipment }] = await Promise.all([
    customerId
      ? supabase.from("customers").select("*").eq("customer_id", customerId).maybeSingle()
  ...
  const payload = buildFraudMlPayload({ order, customer, shipment });
  const apiResult = runMlPredictAsApi(payload);
  const { fraud, probability } = fraudScoresFromMlResult(apiResult);
